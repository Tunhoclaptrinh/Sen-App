...
const GamePlayScreen = () => {
    ...
    useEffect(() => {
        if (currentSession?.collected_items) {
            setLocalCollectedItems(currentSession.collected_items);
        }
    }, [currentSession]);
        ...
        try {
            ...
            if (response.success) {
                if (response.data.finished) {
                    // Level Completed
                    Alert.alert("Level Completed!", `Score: ${response.data.total_score}`, [
                        { text: "OK", onPress: () => navigation.goBack() }
                    ]);
                } else {
                    dispatch(updateCurrentScreen(response.data.next_screen));
                }
            } else {
    ...
    const handleCollectItemWrapper = async (itemId: string) => {
        ...
        try {
            ...
            if (response.success) {
                Alert.alert("Found!", `${response.data.item.name}: ${response.data.item.fact_popup}`);
