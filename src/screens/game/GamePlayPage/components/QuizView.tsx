...
interface QuizOption {
    text: string;
    explanation?: string;
    is_correct?: boolean; // Hidden ideally
}
...
const QuizView: React.FC<QuizViewProps> = ({ data, onSubmit, onNext }) => {
    ...
    const handleOptionPress = async (optionText: string) => {
        ...
        try {
            ...
            // res.data contains { is_correct, explanation, correct_answer }
    ...
    const getOptionStyle = (optionText: string) => {
        ...
        if (optionText === selectedOption) {
            return result.is_correct ? styles.optionCorrect : styles.optionWrong;
        }
        
        if (result.correct_answer === optionText) {
    ...
    return (
        <ScrollView contentContainerStyle={styles.container}>
            ...
            <View style={styles.optionsContainer}>
                {data.options?.map((opt: QuizOption, index: number) => (
                    ...
                    >
                        <Text style={[
                            styles.optionText,
                            (result && (opt.text === selectedOption || opt.text === result.correct_answer)) && styles.optionTextWhite
                        ]}>
                            {opt.text}
                        </Text>
            ...
            {result && (
                <View style={[styles.feedbackContainer, result.is_correct ? styles.feedbackCorrect : styles.feedbackWrong]}>
                    <Text style={styles.feedbackTitle}>
                        {result.is_correct ? "Correct!" : "Oops!"}
                    </Text>
                    {result.explanation && (
